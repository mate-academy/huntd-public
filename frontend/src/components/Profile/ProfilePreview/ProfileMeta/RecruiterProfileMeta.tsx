...
export const RecruiterProfileMeta = React.memo<Props>((props) => {
  ...
  return (
    <ul className={styles.metaWrapper}>
      {items.map((item) => item.name === RecruiterProfileMetaItems.City ?
        <li key={item.name} className={cn(styles.recruiterMetaItem, typography.smallHeading, 'c-semidark-chocolate')}>
          <IconLocation />
          {item.text}
        </li>
        : (
